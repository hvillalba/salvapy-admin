 ...
 */
public class AcercaDeView  extends CssLayout implements View{ 
    public static final String VIEW_NAME = "Requisitos";
    Grid<Acercade> gridAcercaDe = new Grid<>(Acercade.class);
    private Button newCiudad;
    AcercaDeDao acercaDeDao = ResourceLocator.locate(AcercaDeDao.class);
    Logger log =Logger.getLogger("Requisitos");
    List<Acercade> lista = new ArrayList<>();
    private TextField filterText = new TextField("Filtros");
    private AcercaDeForm acercaDeForm = new AcercaDeForm();
    
    public AcercaDeView(){
        try {
            System.out.print("Nueva instancia de Requisitos");
            setSizeFull();
            addStyleName("crud-view");
            
            HorizontalLayout horizontalLayout = createTopBar();
            lista = acercaDeDao.getAcercaDe();
            
            newCiudad.addClickListener(e -> {
                gridAcercaDe.asSingleSelect().clear();
                acercaDeForm.setCiudad(new Acercade());
            });
            ...
            gridAcercaDe.addColumn(Acercade::getId).setCaption("Id");
            gridAcercaDe.addColumn(Acercade::getDescripcion).setCaption("Descripción");
